...
- **GO-LIVE DONE (2026-06-03, session 6):** the Plan 2b Part B batch is complete and the full app is **deployed live at `https://artifact.host`**. DNS delegated to Vercel nameservers (DNSSEC removed at Squarespace — required, Vercel DNS doesn't support it); prod env repointed (`APP_BASE_URL=https://artifact.host`, `NEXT_PUBLIC_SUPABASE_ANON_KEY` added); Supabase OAuth 2.1 server + DCR enabled, consent path `/oauth/consent`, Site URL `https://artifact.host`, redirect allow-list `https://artifact.host/**` (+ `http://localhost:3000/**` for dev); Google + GitHub OAuth apps created (callback `https://bjztcxpqchwpdsrgapqp.supabase.co/auth/v1/callback`) and enabled in Supabase. Supabase signs with **ES256 asymmetric** keys (JWKS verified live). Deployed current `main` via `vercel deploy --prod` (deployment `dpl_8RoKMMzYMj6EeB6RqF75MrPUg4UG`). Verified live: homepage branded, `/docs`/`/dashboard`/`/oauth/consent` → 200, protected-resource metadata returns `{resource: https://artifact.host, authorization_servers:[…/auth/v1]}` with issuer matching the verifier (no trailing slash). **Remaining = two human-in-browser checks only:** (1) web dashboard — Sign in with Google/GitHub at `https://artifact.host/dashboard`, confirm you land back signed in and see your artifacts; (2) MCP client (e.g. Claude) — add `https://artifact.host/mcp`, confirm the browser consent → sign-in → owned deploy (`owner_id` set), and that anonymous deploy still works.
- **Next unstarted work:** none gating. Optional: the two browser checks above; push to a remote (still local-only); roadmap items in `docs/ROADMAP.md`; the password-re-entry edit-page papercut.
